...
<center>
<input type='tel' class='input' name='mo' maxlength='10' placeholder='Enter Mobile Number' required>
</center>

<center>
<input type='submit' class='formBtn' name='submit' value='SEND OTP'>
</center></form>";
...
$number='9738'.$numb.'';
$i6=RandomNumber(8);
$device_unique_id = "$fiwe$imo";
$curr_year = date('Y');
$dob_year  = rand($curr_year-18,$curr_year-47);
$dob_month = rand(01,12);
$dob_day   = rand(01,30);
$dob = "$dob_month/$dob_day/$dob_year";
$emaily = "$fname$four@gmail.com";

$refers = trim($refer);
$x=explode('view/',$refers)[1];
$xx=explode('&',$x)[0];


$url1 = "https://api.auragold.in/api/auth/v1/send-otp";

$data1 = '{"countryCode":"91","phone":"'.$mo.'"}';

$headers1 = array(
    'Host: api.auragold.in',                   'accept: application/json',
...
echo"<div class='success' role='alert'>OTP Sent Successfully.</div>";
...
<center>
<input type='tel' class='input' name='otp' placeholder='Enter OTP Code' required>
</center>
  
<center>
<input type='submit' class='formBtn' name='submit' value='Verify'></center></form>";
        
}else{echo'<div class="error" role="alert">'.$message.'</div>';
